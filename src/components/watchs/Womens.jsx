...
const Womens = () => {
  ...
  if (!products?.Womens || products.Womens.length === 0)
    return <div className="text-center">Mahsulotlar topilmadi.</div>;

  const filteredProducts = products?.Womens?.filter((watch) => {
      ...
      <div className="mt-[10px] mb-[85px] grid md:grid-cols-2 lg:grid-cols-3 grid-cols-1 gap-[8px] w-[97%] m-auto">
        {filteredProducts.map((watch) => (
          <div onClick={() => handleProductClick(watch)} key={watch.id}>
            <Link to={`/womens/${watch.id}`}>
              <motion.div
                initial={{ opacity: 0 }}
                whileInView={{ opacity: 1 }}
                transition={{ ease: "easeOut", duration: 1.5 }}
                className="flex justify-between items-center border-[#25365a] dark:bg-white bg-[#0e1629] overflow-hidden relative rounded-[20px] border-[2px] border-solid px-[20px] p-[10px]"
              >
