...
const Omega = () => {
  ...
  if (!products?.omega || products.omega.length === 0)
    return <div>Maurice mahsulotlari topilmadi.</div>;

  const filteredProducts = products?.omega?.filter((watch) => {
                ...
                <div className="flex flex-col justify-between gap-[23px]">
                  ...
                  >
                    <img
                      src={watch.logo}
                      alt="logo-brend"
                      className={` w-[70px] ${watch.logoPa}  rounded-lg object-cover`}
                    />
                  </motion.div>
                  <motion.div
                    initial={{ opacity: 0, x: -50 }}
