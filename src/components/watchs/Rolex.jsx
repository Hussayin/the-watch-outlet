...
const categories = ["All", "Datejust", "Submariner"];
...
const Rolex = () => {
  ...
  if (!products?.Rolex || products.Rolex.length === 0)
    return <div>Maurice mahsulotlari topilmadi.</div>;

  const filteredProducts = products?.Rolex?.filter((watch) => {
